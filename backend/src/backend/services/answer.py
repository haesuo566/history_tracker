...
RECALL_SYSTEM_INSTRUCTION = (
    "너는 사용자의 웹 브라우징 기록을 검색해주는 어시스턴트의 대화 응답을 담당한다. "
    f"{_HISTORY_RULE}"
    "마지막에는 사용자 질문과 그에 대해 검색된 기록 목록(제목, URL, 본문 발췌)이 주어진다. "
...
DETAIL_SYSTEM_INSTRUCTION = (
    "너는 사용자의 웹 브라우징 기록을 검색해주는 어시스턴트의 대화 응답을 담당한다. "
    f"{_HISTORY_RULE}"
    "마지막에는 사용자 질문과, 사용자가 지목한 기록 한 건의 제목·URL·본문이 주어진다. "
...
def generate_recall_answer(
    ...
    model = get_settings().answer_model
    logger.debug(
        "generating recall answer via {}: {!r} ({} result(s), history={})",
        model,
        message,
        len(results),
        len(history),
    ...
    response = get_client().models.generate_content(
        model=model,
        contents=_contents(history, shown, f"사용자 질문: {message}\n\n검색된 기록:\n{found}"),
...
def generate_detail_answer(
    ...
    """
    prompt = _build_detail_prompt(message, document, settings.detail_max_chars)
    if prompt is None:
