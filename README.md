...
청킹(임베딩 입력 한계에 맞춰 자동, 개행 경계) → 임베딩(Gemini 또는 TEI)
     ...
     │
     │  POST /chat  { message: "어제 본 리액트 글", client_now: "2026-08-31T12:00:00+09:00" }
     ▼
전처리: 대화 확정·입력 저장 → 직전 대화 + 직전에 보여준 결과 목록을 맥락으로
        의도 판단 + 질의 재작성 + 지목 대상 번호 + 기간을 한 번에 받음 (Gemini 1회)
        기간은 라벨("어제")로 받아 서버가 실제 구간으로 바꿈
     │
     ├─ etc     잡담. 검색 없이 일반 응답
     │
     ...
     │
     └─ recall  기록을 새로 검색 (기간을 말했으면 그 기간에 수집된 문서만)
            │
            ...
            기간만 말하고 찾을 낱말이 없으면("어제 본 거 다 보여줘")
            검색을 건너뛰고 그 기간의 문서를 최근 순으로 나열
```
...
| `POST` | `/chat` | `{ "message": "...", "conversation_id": "..." \| null, "client_now": "2026-08-31T12:00:00+09:00" \| null }` | `{ conversation_id, results: [{ document_id, url, title, score, snippet, visited_at }], answer }` |
...
`client_now` 는 **오프셋이 붙은** 클라이언트의 현재 시각입니다. "어제 본 글" 같은 질의의 하루
경계는 사용자의 자정이라 서버 시각으로는 알 수 없어, 브라우저가 실어 보냅니다. 없어도 기간 없는
질의는 그대로 동작하며, 기간을 풀어야 하는데 없으면 UTC 자정을 경계로 삼고 경고를 남깁니다.
`visited_at` 은 그 페이지를 수집한 시각으로, UTC 임을 밝혀(`...Z`) 내보냅니다.
...
찾는 일과 이 턴이 `recall` 인지 판단하는 일이 같은 추론이라, 의도·검색어·지목 대상 번호·기간·요청
개수를 한 번의 호출로 함께 받습니다.

## 시간 기반 검색

"어제 본 리액트 글", "지난주에 봤던 거", "8월 20일에 본 페이지"처럼 기간을 말하면 그 기간에
수집된 문서만 검색합니다. 재작성 호출이 기간을 **라벨**(`yesterday`, `last_week`, `recent_days`
…)로 고르고, 그것이 실제로 며칠인지는 서버가 정합니다(`services/timerange.py`). 날짜 계산을
모델에게 맡기면 틀려도 티가 나지 않은 채 엉뚱한 기간을 뒤지기 때문입니다. 날짜를 직접 말한
경우(`8월 20일`)만 모델이 적은 값을 씁니다.

| 규칙 | 값 |
| --- | --- |
| 경계 | `since` 이상 `until` 미만 (자정이 두 구간에 겹쳐 들지 않도록) |
| 주의 시작 | 월요일 |
| "최근 3일" | 오늘을 포함한 사흘 |
| 끝 | 오늘까지로 당김 (아직 오지 않은 날에는 기록이 없음) |
| 기준 시각 | 요청의 `client_now`, 없으면 UTC |

기간이 걸린 검색이 0건이어도 **기간을 풀어 다시 찾지 않습니다.** 사용자는 "어제 본 것"을
물었고, 그때 그저께 것을 내놓으면 어제 본 것이라고 오해한 채로 읽습니다. 대신 답변이 어느
기간을 뒤졌는지 밝힙니다.

기간만 말하고 찾을 낱말이 없는 질의("어제 본 거 다 보여줘")는 검색을 건너뛰고 그 기간의 문서를
최근 순으로 나열합니다(기본 10건). 그대로 검색을 태우면 명사가 없어 전문 검색은 빈손이고 벡터
검색은 아무 문서나 끌어옵니다.

기준이 되는 시각은 `documents.timestamp`, 즉 **그 URL 을 처음 수집한 때**입니다. 같은 URL 을
다시 방문해도 갱신되지 않으므로(중복 판정이 URL 단위) 재방문은 기간 검색에 반영되지 않습니다.
